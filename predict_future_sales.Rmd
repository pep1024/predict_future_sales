...
test_df <- data.frame(month = 34, q = length(unique(test$shop_id)))
...
```{r}
sum(!unique(test$shop_id) %in% unique(dataset_all[dataset_all$date_block_num == 33, ]$shop_id))
```
...
plot(df_unique_shop_item_by_month$month, df_unique_shop_item_by_month$q,
  ...
  xlim = c(0, 34))
# Add shops in the test dataset
test_df <- data.frame(month = 34, q = max(test$ID) + 1)
points(test_df$month, test_df$q, pch = 16, col = "red")
grid()
```
...
plot(df_q_months_by_shop$shop, df_q_months_by_shop$q,
  ...
  xlim = c(0, 59))
# Add shops in the test dataset
test_df <- data.frame(shop = 0:59, q = (0:59) %in% unique(test$shop_id))
...
```{r}
month <- 23
test_set <- dataset_all[dataset_all$date_block_num == month, ]
all_combinations <- expand.grid(shops = unique(test_set$shop_id), item = unique(test_set$item_id))
```
...
In Nov-2015, we have `r nrow(testset)`. Total number of shop_item in month 34 that does not exist in month 33 are
...
cat("items in month - 1 (", month - 1, "):", m_in_previous_month[month, ], "\n")
cat("items not in month -1 (", month - 1, ") but in some month before:", 
