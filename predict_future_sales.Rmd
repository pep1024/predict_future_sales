...
plot(test_shop$series, 
  xlim = c(2014, 2016), typ = "b", pch = 1, ylim = c(0, 1.1 * max(test_shop$series)),
  las = 1, ylab = "Quantity", xlab = "Month")
#x_ticks <- axis.Date(1, test_shop$series, labels = F)
#time(ts(0, 2014, 2016, frequency = 12))

#axis(1, seq(as.Date("2014-01-01"), as.Date("2016-01-01"), by = "month"))

points(test_shop$prediction, pch = 16, col ="red")
#points(tail(time(shop_set_by_month.ts), 1), tail(shop_set_by_month.ts, 1), pch = 16, col ="black")
points(time(test_shop$series)[month_forecast + 1], test_shop$series[month_forecast + 1],
  pch = 16)
```
